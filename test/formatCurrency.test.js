...
test('formats 100 to "€ 100,00"', (t) => {
	const locale = new VI18N('nl-NL', 'EUR');
...
test('formats 1 to "€ 1"', (t) => {
	const locale = new VI18N('nl-NL', 'EUR', { currency: { minimumFractionDigits: 0, maximumFractionDigits: 0 } });
...
test('formats 100 US dollars to "US$ 100,00"', (t) => {
	const locale = new VI18N('us-US', 'USD');

	t.is(locale.formatCurrency(100), 'US$ 100,00');
});

test('formats 100 Japanese yen to "¥ 100"', (t) => {
	const locale = new VI18N('ja', 'JPY');

	t.is(locale.formatCurrency(100, yen), '￥100');
});
